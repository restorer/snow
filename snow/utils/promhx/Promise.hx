...
#end
import snow.utils.promhx.base.EventLoop;
import snow.utils.promhx.base.AsyncBase;
import snow.utils.promhx.error.PromiseError;

@:expose
class Promise<T> extends AsyncBase<T>{
    var _rejected   : Bool;

    public function new( ?d:Deferred<T>, ?func: (T->Void)->(Dynamic->Void)->Void ) {

        var defer = d != null ? d : new Deferred<T>();
    ...
    macro public static function when<T>( args : Array<ExprOf<Promise<Dynamic>>>) : Expr {
        ...
        return macro {
            ...
            var varargf = function(f){
                ...
                var ret = new snow.utils.promhx.Promise();
                var arr : Array<snow.utils.promhx.Promise<Dynamic>> = $eargs;
                var p = snow.utils.promhx.Promise.whenAll(arr);
                p._update.push({
                    async : ret,
